...
const ProjectCards = ({ name, description, tags, image, image2, mobImg, mobImg2, index, source_code_link, hosted_site_link, download_app }) => {
  ...
  const handleNoteAppDownload = () => {
    console.log("app downloading...")
    try {
      const fileUrl = '/reactnative-app.apk'
      const link = document.createElement('a')
      link.href = fileUrl
      link.setAttribute('download', 'Google Notes Clone.apk')
    ...
    <div className={` sm:max-h-[400px] dark:text-white text-[#444444] flex flex-col sm:flex-row justify-center rounded-3xl mx-7 my-4 relative overflow-hidden ${index >= 0 ? (bgColors[index]) : "bg-red-500"} ${isLeftAligned ? '' : 'sm:flex-row-reverse'}`}>
      ...
      {mobImg &&
        <motion.div
          ...
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
          className={`flex ${isLeftAligned ? ' justify-start' : 'justify-end'} relative`}
        >
          <img src={mobImg} alt='mobile' className='max-w-full h-full sm:py-4 py-2 object-scale-down z-10' />

          {mobImg2 &&
            <motion.div
              initial={{x: 0, opacity: 0}}
              transition={{ duration: 0.5, delay: 1.5 }}
              whileInView={{ x: 160, opacity: 1 }}
              // viewport={{ once: true }}
              className=' absolute bottom-6 right-[12%] sm:mb-2 z-0'
            >
              <img src={mobImg2} alt='mobile' className=' max-w-[80%] inset-0' />
            </motion.div>
          }
        </motion.div>
      }
